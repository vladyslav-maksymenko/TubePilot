...
internal sealed class TelegramProcessingQueue
{
    private readonly ILogger<TelegramProcessingQueue> _logger;
    private readonly int _maxConcurrentJobs;
    private readonly object _gate = new();
    private readonly Dictionary<long, ChatQueueState> _chatStates = [];

    public TelegramProcessingQueue(int maxConcurrentJobs, ILogger<TelegramProcessingQueue> logger)
    {
        _logger = logger;
    ...
    private async Task<QueueAdmission> QueueOnBackgroundAsync(QueueEntry entry, int queuedPosition, bool shouldQueue)
    {
        if (shouldQueue)
        {
            ...
            {
                _logger.LogDebug(ex, "Telegram queued-state update failed for chat {ChatId}, message {MessageId}.", entry.ChatId, entry.MessageId);
            }
        }
    ...
    private async Task RunEntryAsync(QueueEntry entry)
    {
        try
        {
            ...
            {
                _logger.LogDebug(ex, "Telegram start-state update failed for chat {ChatId}, message {MessageId}.", entry.ChatId, entry.MessageId);
            }
        ...
        {
            _logger.LogDebug("Processing job canceled for chat {ChatId}, message {MessageId}.", entry.ChatId, entry.MessageId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Processing job failed for chat {ChatId}, message {MessageId}.", entry.ChatId, entry.MessageId);
        }
        finally
        {
